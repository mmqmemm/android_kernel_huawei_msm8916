...
{
	pr_debug("%s,otp_ptr.product_year:0x%x",__func__,otp_ptr.product_year);
	pr_debug("%s,otp_ptr.product_month:0x%x",__func__,otp_ptr.product_month);
	pr_debug("%s,otp_ptr.product_date:0x%x",__func__,otp_ptr.product_date);
	pr_debug("%s,otp_ptr.camera_id:0x%x",__func__,otp_ptr.camera_id);
	pr_debug("%s,otp_ptr.supplier_version_id:0x%x",__func__,otp_ptr.supplier_version_id);
	pr_debug("%s,otp_ptr.version:0x%x\n",__func__,otp_ptr.version);
	pr_debug("%s,otp_ptr.wb_rg:0x%x",__func__,otp_ptr.wb_rg);
	pr_debug("%s,otp_ptr.wb_bg:0x%x",__func__,otp_ptr.wb_bg);
	return;
}
/******************************************************************************
...
void s5k4e1_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl,
                                             ...
                                             uint32_t b_gain)
{
	pr_debug("%s, R_gain:0x%x,G_gain:0x%x,B_gain:0x%x\n",__func__, r_gain, g_gain, b_gain);
	if (r_gain >= 0x100) {
                                          ...
                                          int isMsb)
{
	...
	pr_debug("%s reg:0x%x, isMsb:%d ", __func__, reg, isMsb);
	...
	for (layer = S5K4E1_LAYER_START; layer <= S5K4E1_LAYER_AWB_END; layer++)
	{
		...
		pr_debug("%s reg_values[%d]:0x%x", __func__, i, reg_values[i]);
	}
                                                ...
                                                struct s5k4e1_otp_struct *current_otp)
{
	...
	{
		pr_err("%s Error! Invalid OTP location:%d \n", __func__, location);
		rc = -1;
		return rc;
	}

	pr_debug("%s location:%d \n", __func__, location);
...
s5k4e1_otp_location s5k4e1_check_awb_otp_location(struct msm_sensor_ctrl_t * s_ctrl)
{
	...
	pr_debug("%s read layer %d: reg310d:0x%x, reg310e:0x%x, reg310f:0x%x \n", __func__, layer_data, reg310d, reg310e, reg310f);
...
static int s5k4e1_update_otp (struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	{
		pr_err("%s Error!no AWB data in OTP!\n", __func__);
		return -1;
	}
	...
	if ((0 == current_otp.wb_rg) || (0 ==current_otp.wb_bg))
	{
		pr_err("%s Error!Zero RG or BG data! RG:0x%x, BG:0x%x!\n", __func__, current_otp.wb_rg, current_otp.wb_bg);
		return -1;
	}
	/*calculate*/
...
int s5k4e1_otp_func(struct msm_sensor_ctrl_t *s_ctrl, int index)
{
	...
	if (0 == (s5k4e1_otp_read_flag & S5K4E1_OTP_AWB))
	{
		if(0 != s5k4e1_update_otp(s_ctrl))
		{
			...
			pr_err("%s : No OTP info! return error!\n", __func__);
			return -1;
		}
	}
	else
	{
		pr_debug("%s :AWB OTP has already been read out!\n",__func__);
	}
	/*write*/
